package com.redis.bottleneck.service.per;
...
public class RedisCachePERService implements RedisCacheService {
    ...
    @Override
    @Cacheable(
            cacheStrategy = CacheStrategy.PER,
            cacheName = "article", //PER:article:~~
            key = "#articleId",
            ttl = 5
    )
    ...
    public ArticlePageResponse readAllInfiniteScroll(Long boardId, Long lastArticleId, Long pageSize) {
        return articleService.readAll(boardId, lastArticleId, pageSize);
    }
